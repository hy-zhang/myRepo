...
\textbf{(3).} ??? (I don't think this is correct if $A$ is not full-rank.)\\
...
\textbf{Necessity.} ??? (How to prove it?)
...
??? (Q: How to solve it?) (A: Use derivative.)\\
...
\textbf{(c).} Slater's condition doesn't hold for this problem, since with the constraint $y>0$, $x^2/y<0$ is not feasible. (??? Right?)\\
